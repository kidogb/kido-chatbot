...
def classify_train():
    ...
    goodbye = []
    food = []
    greeting = open("greeting.txt","r").read().split('\n')
    goodbye = open("goodbye.txt","r").read().split('\n')
    food = open("food.txt","r").read().split('\n')

    for sentence in food:
        training_data.append({"class":"food", "sentence":sentence})
    for sentence in greeting:
        training_data.append({"class":"greeting", "sentence":sentence})
    for sentence in goodbye:
        training_data.append({"class":"goodbye", "sentence":sentence})
...
def chat():
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            TYPE = classify(line)[0]
            print (TYPE)
            if TYPE == "greeting" or TYPE == "goodbye" or TYPE == "food":
                aiml_respond = kernel.respond(line)
                if aiml_respond != '': #if dont have aiml_respond, use rnn instead.
                    print(aiml_respond)
                    output_file.write('BOT ++++ ' + aiml_respond + '\n')
                else:
